...
\end{frontmatter}
	...
	\begin{itemize}
		...
		\item Target selection refers to which specific results (out of multiple original results) a redoing activity should focus on.



		\item Issues with data and code relate to their lack of availability, provenance, correctness, and executability.



		\item Evaluation of results concerns how original and reproduction results are compared and what the conclusion is.



		\item Synthesis of results involves integrating and reporting multiple results, both on the level of individual studies and across them.
...
\end{landscape}
	...
	Given one or multiple \textsc{descriptions}, all \textsc{choices left unspecified}, and, potentially, \textsc{data}, one can infer a particular \textsc{computation} (see S3 \rightarrow{ }n\textsubscript{1}).
	...
	\end{itemize}
	\emph{\textbf{Primary collected data}} (see S7 \rightarrow{ }n\textsubscript{5}): First digital representation of the \textsc{raw collected data} (Gollwitzer et al., 2021).



	\noindent\emph{\textbf{Raw collected data}} (see S7 \rightarrow{ }n\textsubscript{4}): Original records, that is the first non-volatile form of \textsc{collected data}. In the case of electronic data collection, raw collected data and primary collected data coincide (Gollwitzer et al., 2021).
	...
	\begin{itemize}[label=\phantom{\textbullet},topsep=0pt]
		\item \textbf{\emph{Reported code}} (see S5 \rightarrow{ }n\textsubscript{4}): \textsc{\emph{Code}}\emph{ made available by its authors.}
	...
	Taken together, we provide a conceptual framework to describe computational reproductions and their epistemic function of verification in detail. Revisiting the challenges described in Section 1, the procedure we recommend addresses them in several ways: 
	...
	Arguillas, F., Christian, T.-M., Gooch, M., Honeyman, T., Peer, L., \& CURE-FAIR WG. (2022, July). \emph{10 things for curating reproducible and FAIR research} (Version 1.1). \url{https://doi.org/10.15497/RDA00074}
	...
	Crook, S. M., Davison, A. P., \& Plesser, H. E. (2013). Learning from the past: Approaches for reproducibility in computational neuroscience. In J. M. Bower (Ed.), \emph{20 Years of Computational Neuroscience} (pp. 73--102). \url{https://doi.org/10.1007/978-1-4614-1424-7\_4}
